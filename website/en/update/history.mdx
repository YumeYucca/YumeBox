...
<Update label="2026-9-19" tags={["Feature updates","optimization","repair"]} description="v0.6.4">
## Change

- Refactored proxy delay tests and health checks: group-level overlay, progress display, and coalesced delay refresh
- Combined node search, sorting, and delay test actions in the proxy toolbar, with smoother list updates while testing
- Improved health checks for preview runtime and when the proxy is stopped, with unified delay results
- Merged remote-controller reachability probing with non-blocking proxy refresh to reduce stalls in remote mode
- Removed the Moe Home wallpaper bottom-dim toggle; the bottom fade is always applied so rounded corners do not reveal the wallpaper
- Improved country/region flag display logic
- Updated the Android NDK and Go module dependencies
...
- Added a [web-based custom icon builder](https://yumebox.yumeyuka.moe/guide/icon-builder) that lets users upload and customize app icons, then export an Android icon ZIP or submit a build request
